...
const Trending = () => {
  ...
  const { data, error, isLoading } = useQuery<NowPlayingResponse>({
    ...
    queryFn: () => getTrendingMovies("day", page),
    staleTime: 30000,
  });
  ...
  useEffect(() => {
    const cachedData = localStorage.getItem(`trendingMovies_page_${page}`);
    if (cachedData) {
  ...
  useEffect(() => {
    if (data) {
      localStorage.setItem(`trendingMovies_page_${page}`, JSON.stringify(data));
      setCachedData(data);
    }
  }, [data, page]);
